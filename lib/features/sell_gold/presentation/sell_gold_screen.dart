...
class _SellGoldScreenState extends ConsumerState<SellGoldScreen> {
  ...
  void _onAmountChanged(String val) {
    ...
    ref.read(sellGoldNotifierProvider.notifier).updateAmount(parsed);

    final state = ref.read(sellGoldNotifierProvider);
  ...
  void _onQuantityChanged(String val) {
    if (_ignoreControllerCallbacks) return;
    final parsed = double.tryParse(val) ?? 0.0;
    ref.read(sellGoldNotifierProvider.notifier).updateQuantity(parsed);
  ...
  void _onSellMax() {
    ref.read(sellGoldNotifierProvider.notifier).selectMax();
    final state = ref.read(sellGoldNotifierProvider);
    _ignoreControllerCallbacks = true;
    _quantityController.text = state.goldQuantity.toStringAsFixed(4);
    _amountController.text = state.amount.toStringAsFixed(2);
          ...
          return ResponsivePage(
            ...
            children: [
                  ...
                  child: Row(
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          Text('Available Gold Balance', style: textTheme.bodyMedium?.copyWith(color: Colors.grey)),
                          const SizedBox(height: 4),
                          walletAsync.when(
                            loading: () => const Text('Loading...', style: TextStyle(fontWeight: FontWeight.bold)),
                            error: (_, __) => const Text('Unavailable', style: TextStyle(fontWeight: FontWeight.bold)),
                            data: (wallet) => Text(
                              '${wallet.availableGold.toStringAsFixed(4)} grams',
                      ...
                      OutlinedButton.icon(
                        onPressed: _onSellMax,
                        icon: const Icon(Icons.bolt, size: 16),
                        label: const Text('Sell Max'),
                      ),
                    ],
                  ),
                ),
              ...
              FilledButton(
                ...
                onPressed: sellState.error != null || sellState.goldQuantity <= 0 || sellState.submitting
                    ? null
                    : () {
