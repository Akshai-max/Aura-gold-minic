...
final inventorySkipProvider =
    NotifierProvider<InventorySkipNotifier, int>(InventorySkipNotifier.new);
...
final inventoryLimitProvider =
    NotifierProvider<InventoryLimitNotifier, int>(InventoryLimitNotifier.new);
...
final inventoryListProvider =
  ...
  final response =
      await apiClient.get('/inventory/', queryParameters: params);
  return PaginatedInventoryItems.fromJson(response.data as Map<String, dynamic>);
});

final inventoryMetricsProvider =
    FutureProvider.autoDispose<InventoryMetrics>((ref) async {
  final apiClient = ref.watch(apiClientProvider);
...
final inventoryMovementsProvider = FutureProvider.autoDispose
  ...
  return PaginatedStockMovements.fromJson(response.data as Map<String, dynamic>);
});
...
final stockInProvider = Provider<
  ...
  return (itemId, quantity, {reference, notes, supplierId}) async {
    ...
    final response =
        await apiClient.post('/inventory/$itemId/stock-in', data: data);
...
final stockOutProvider = Provider<
    Future<InventoryItem> Function(
      String itemId,
      int quantity, {
      String? reference,
  ...
  return (itemId, quantity, {reference, notes}) async {
    ...
    if (notes != null) data['notes'] = notes;
    final response =
        await apiClient.post('/inventory/$itemId/stock-out', data: data);
    ref.invalidate(inventoryDetailProvider(itemId));
    ref.invalidate(inventoryMovementsProvider(itemId));
    ref.invalidate(inventoryListProvider);
    ref.invalidate(inventoryMetricsProvider);
...
final stockAdjustProvider = Provider<
    Future<InventoryItem> Function(
      String itemId,
      int newQuantity, {
      String? reason,
  ...
  return (itemId, newQuantity, {reason, notes}) async {
    ...
    if (notes != null) data['notes'] = notes;
    final response =
        await apiClient.post('/inventory/$itemId/stock-adjust', data: data);
    ref.invalidate(inventoryDetailProvider(itemId));
    ref.invalidate(inventoryMovementsProvider(itemId));
    ref.invalidate(inventoryListProvider);
    ref.invalidate(inventoryMetricsProvider);
...
final suppliersListProvider =
    FutureProvider.autoDispose<PaginatedSuppliers>((ref) async {
  final apiClient = ref.watch(apiClientProvider);
...
final createSupplierProvider =
    Provider<Future<Supplier> Function(Supplier)>((ref) {
...
final globalMovementsProvider =
  ...
  final apiClient = ref.watch(apiClientProvider);
  final response = await apiClient.get('/inventory/movements', queryParameters: {
    'limit': 50,
  });
  return PaginatedStockMovements.fromJson(response.data as Map<String, dynamic>);
});
...
final suppliersSkipProvider =
    NotifierProvider<SuppliersSkipNotifier, int>(SuppliersSkipNotifier.new);
