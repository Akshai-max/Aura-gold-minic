...
async def seed_data(session: AsyncSession) -> None:
    ...
    # 2. Define Roles
    roles_list = [
        {"name": "super_admin", "description": "Full system access permissions", "permissions": []},
        {"name": "admin", "description": "Administrative access permissions", "permissions": []},
        {"name": "user", "description": "Standard user access permissions", "permissions": []},
    ]
    
    db_roles = {}
    for role_data in roles_list:
        query = select(Role).where(Role.name == role_data["name"]).options(selectinload(Role.permissions))
        result = await session.execute(query)
    ...
    query = select(User).where(User.email == admin_email).options(selectinload(User.roles))
    result = await session.execute(query)
    ...
    logger.info("database_seeding_complete", message="Database seeding completed successfully.")
