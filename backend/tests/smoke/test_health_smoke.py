...
@pytest.mark.asyncio
async def test_smoke_health_endpoint(smoke_client: httpx.AsyncClient):
...
@pytest.mark.asyncio
async def test_smoke_openapi_available(smoke_client: httpx.AsyncClient):
...
@pytest.mark.asyncio
async def test_smoke_login_and_me(smoke_client: httpx.AsyncClient):
    """Verify admin can log in and fetch profile (post-deploy smoke)."""
    login_response = await smoke_client.post(
        "/api/v1/auth/login",
    ...
    me_response = await smoke_client.get(
        "/api/v1/auth/me",
